...
except ImportError:  # : slimmed environment has no loguru, the VAE does not actually use it, safe to degrade gracefully
    loguru = None
import numpy as np
import torch
...
# vendored into hunyuan/ (self-contained); imports changed to be package-internal
...
def forward_with_checkpointing(module, *inputs, use_checkpointing=False):
    """Run module forward, optionally with gradient checkpointing."""
...
class AutoencoderKLConv3D(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        """Full autoencoder forward: encode to latent, sample/mode, decode."""
