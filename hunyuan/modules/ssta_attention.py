...
def tile(x, canvas_thw, tile_thw, sp_size=1):
    r"""Rearrange tensor (b, head, s, d) into tiles for block-based attention."""
    b, h, s, d = x.shape
    t, h, w = canvas_thw
...
def importance_sampling(q, k, topk, threshold=0.0, lambda_=0.9, adaptive_pool=None):
    r"""Select top-k blocks based on importance scores (similarity and redundancy)."""
    if threshold > 0.0:
...
def ssta_3d_attention(all_q, 
                     ...
                     sampling_type=None,
                     adaptive_pool=None):
    r"""Sparse Spatio-Temporal Attention (SSTA) 3D attention; returns (output, sparse_ratio)."""
    try:
