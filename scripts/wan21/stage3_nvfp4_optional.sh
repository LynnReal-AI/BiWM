...
#   generator's nn.Linear fake-quantized to NVFP4 (block16, weight+activation), runs NVFP4 throughout (student);
#   additionally adds BF16<->NVFP4 self-distillation KL (flow_kl + x0_kl) to align the quantized output to its own full precision, resisting quantization quality loss/purple tint.
#   * Inference real speedup requires Blackwell(GB200) + a real FP4 GEMM kernel/export; this script's fake-quant is only for QAT.
# =============================================================================
...
#   kl_weight: total self-distillation KL weight (empirically: 0.1/0.5 too strong -> purple tint, 0.03 recommended to start)
#   x0_weight: the x0 term of KL (directly constrains low-frequency/color, 0.25 resists purple tint); flow_weight: velocity term (1.0)
#   warmup:    first N steps without KL (let the generator stabilize under quantization first; 0 = add from the start)
...
echo "=============================================="
echo "Wan 2.2 5B - Stage 3 NVFP4 quantization-aware training (QAT, inherits stage-2 DMD) on BiWM"
