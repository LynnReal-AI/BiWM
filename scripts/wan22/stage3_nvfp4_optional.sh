#!/usr/bin/bash
# ★★ This script = stage2_dmd.sh + NVFP4 quantization-aware training (QAT) ★★
#   The generator's nn.Linear is fake-quantized to NVFP4 (block16, weight+activation), running NVFP4 throughout (student);
#   additionally adds BF16↔NVFP4 self-distillation KL (flow_kl + x0_kl) to align quantized outputs to full precision, fighting quantization degradation / purple tint.
#   ★ Real inference speedup requires Blackwell(GB200) + a real FP4 GEMM kernel/export; the fake-quant in this script is only for QAT.
# =============================================================================
...
#   kl_weight: total self-distillation KL weight (empirical: 0.1/0.5 too strong→purple tint, 0.03 recommended start)
#   x0_weight: the x0 term of KL (directly constrains low frequency/color, 0.25 fights purple tint); flow_weight: velocity term (1.0)
#   warmup:    do not add KL for the first N steps (let the generator stabilize under quantization first; 0=add from the start)
NVFP4_KL_WEIGHT=${NVFP4_KL_WEIGHT:-0.03}
NVFP4_X0_WEIGHT=${NVFP4_X0_WEIGHT:-0.25}
NVFP4_FLOW_WEIGHT=${NVFP4_FLOW_WEIGHT:-1.0}
NVFP4_WARMUP=${NVFP4_WARMUP:-0}
NVFP4="--dmd_nvfp4 --nvfp4_kl_weight ${NVFP4_KL_WEIGHT} --nvfp4_flow_weight ${NVFP4_FLOW_WEIGHT} \
       --nvfp4_x0_weight ${NVFP4_X0_WEIGHT} --nvfp4_warmup_steps ${NVFP4_WARMUP}"
...
# ===== Log =====
LOG_NAME="Wan22_5B_stage3_nvfp4_biwm_480p_77f"
...
echo "=============================================="
echo "Wan 2.2 5B - Stage 3 NVFP4 quantization-aware training (QAT, inheriting second-stage DMD) on BiWM"
...
echo "  NVFP4 QAT         : on (KL w=${NVFP4_KL_WEIGHT} flow=${NVFP4_FLOW_WEIGHT} x0=${NVFP4_X0_WEIGHT} warmup=${NVFP4_WARMUP})"
