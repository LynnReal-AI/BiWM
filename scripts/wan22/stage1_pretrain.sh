...
#       at the latent level (20 latent frames = 1 init + 19 motion segments), no continuous Plucker.
...
# Weights: trained from scratch from the Wan2.2-TI2V-5B base (no resume)
...
# ===== Resolution: 832x480 (divisible by 32 OK: 832/32=26, 480/32=15) =====
NUM_HEIGHT=480
NUM_WIDTH=832

# ===== BiWM dataset path (can be overridden by wrapper scripts via env, e.g. stage1_video_real.sh) =====
BIWM_VIDEO_DIR=${BIWM_VIDEO_DIR:-./dataset/videos}
BIWM_CAPTION_JSON=${BIWM_CAPTION_JSON:-./dataset/videos_syn.json}

# ===== Logging =====
TOTAL_GPUS=$((NNODES * NPROC_PER_NODE))
LOG_NAME=${LOG_NAME:-"wan22_5b_camtext_480p_77f"}
LOG_DIR="./logs/wan22/stage1"
OUTPUT_DIR="./logs/wan22/stage1"
mkdir -p ${LOG_DIR}

LOG_FILE="${LOG_DIR}/${LOG_NAME}_${TOTAL_GPUS}gpus_node${NODE_RANK}_$(date +%Y%m%d_%H%M%S).log"
...
echo "Weights: Wan2.2-TI2V-5B base (from scratch, no resume)"
echo "=============================================="
...
WAN_MODEL_PATH=./ckpts/Wan2.2-TI2V-5B
