...
# =============================================================================
# This script = the FramePack entry of stage2_dmd.sh (thin wrapper, reusing the same training logic/data/inherited weights).
#   dataset/inherited weights already switched to video_real in stage2_dmd.sh (this entry inherits automatically).
...
#     · No LR branch (FramePack tokens are already the full compressed history), uniformly returns (mem_tokens, mem_indices_grid).
#   Implementation: pipelines/wan/history_compress_framepack.py
#
# ★ Relation between compression tier and history length (K=4 → history T=4·num_blocks):
#     2x@T≥3(≈0.4s,blocks≥1)  4x@T≥7(blocks≥2)  8x@T≥23(blocks≥6)  16x@T≥87  32x@T≥343
#   Current default NUM_CHUNKS=5 → max history T=16 (small tier, up to 4x). To trigger 8x and above, increase NUM_CHUNKS≥6.
#
# Usage:
...
# Separate output directory per mode, to avoid ckpts overwriting each other
export LOG_NAME=${LOG_NAME:-"Wan22_5B_stage2_dmd_framepack_video_real_480p_77f"}

# Pass through all logic to the base script (env vars STAGE1_DIR / NUM_CHUNKS / GENERATOR_CKPT etc. also take effect)
