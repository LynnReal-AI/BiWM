...
def _dmd_extract_cam(di):
    ...
    n = len(di)
    if n == 16:
        # BiwmCamCaptionData: 15-field + trailing action_labels
        (_v, _r, cap, K_ctrl, c2w, _id, has, _s, _c, pw, ph, _fs, _fe, _Kr, c2w_raw, action_labels) = di
    elif n >= 15:
        (_v, _r, cap, K_ctrl, c2w, _id, has, _s, _c, pw, ph, _fs, _fe, _Kr, c2w_raw) = di
    elif n >= 13:
        (_v, _r, cap, K_ctrl, c2w, _id, has, _s, _c, pw, ph, _Kr, c2w_raw) = di
    else:
        cap = di[2] if n > 2 else ""
    ...
    return cap, K_ctrl, c2w, c2w_raw, hc, ph, pw, action_labels
    ...
    for epoch in range(10 ** 9):
        ...
        for data_item in train_dataloader:
            ...
            if (_use_gan or _use_gt_reg or _use_sft or _use_real_fkl) and vae_encoder is not None:
                gt_latent = _dmd_embed_gt_latent(data_item[0], vae_encoder, current_device,
