...
def prepare_camera_inputs(
    ...
    K_batch = torch.as_tensor(np.stack(K_vec_list, axis=0), dtype=torch.float32)   # [B, V, 4]
    c2w_batch = torch.as_tensor(np.stack(c2w_all_list, axis=0), dtype=torch.float32)  # [B, V, 4, 4]
    plucker_bhw6 = ray_condition(K_batch, c2w_batch, H, W, device='cpu')  # [B, V, H, W, 6]
...
# 左摇杆: 平移方向 (前进/后退/左/右)
# 右摇杆: 旋转方向 (yaw/pitch)
# 参考: /mnt/workspace/lichuanhao/code/add_vis_to_video_single.py
